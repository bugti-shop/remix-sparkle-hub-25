...
const GRACE_PERIOD_HOURS = 48; // 48-hour grace period
...
export const recordCompletion = async (
  ...
  // Check if earned a freeze (5 tasks in a day) — Premium only
  ...
  if (data.lastCompletionDate === yesterday) {
    ...
    if (withinGrace && !data.gracePeriodUsed && data.currentStreak > 0) {
      ...
      // Can use a freeze (missed exactly one day) — Premium only
      data.streakFreezes -= 1;
      data.currentStreak += 1;
      usedFreeze = true;
      streakIncremented = true;
